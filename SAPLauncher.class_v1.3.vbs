...
'######## Script name: SAPLauncher.class.v1.3 ########
'################## Major version:1 ##################
'################## Minor version:3 ##################
'#################### Version:1.3 ####################
'#####################################################
...
'## Major implementation changes. Less code and should
'## be faster and more reliable
'## Xpath 1.0 used instead of looping through xml nodes
'## manually
'## CheckSAPLogon internals simplified
...
Class SAPLauncher
	...
	Public Function IsSapSystemValid(sysname)
		debug.WriteLine oXML.selectSingleNode("//Landscape/Services/Service[starts-with(@name, '" & sysname & "')]").attributes.getNamedItem("name").text
'		If Not IsNull(oXML.selectSingleNode("//Landscape/Services/Service[starts-with(@name, '" & sysname & "')]").attributes.getNamedItem("name")) Then
'			IsSapSystemValid = True
'		Else
'			IsSapSystemValid = False
'		End If 
	End Function 
